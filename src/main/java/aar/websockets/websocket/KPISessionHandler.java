...
public class KPISessionHandler {
	...
	public static void actualizarValorAleatorio() {
		...
		for (KPI kpi : Kpis) {
			numero = calcularValor();
			JsonObject addMessage = provider.createObjectBuilder().add("action", "actualizar").add("id", kpi.getId())
					.add("comparacion", numero).add("modificado", fechaSistema()).build();
	...
	public void addSession(Session session) {
		...
		JsonProvider provider = JsonProvider.provider();
		for (KPI kpi : Kpis) {
			JsonObject addMessage = provider.createObjectBuilder().add("action", "add").add("id", kpi.getId())
					.add("name", kpi.getName()).add("type", kpi.getType()).add("name2", kpi.getName2())
					.add("type2", kpi.getType2()).add("comparacion", valor).add("modificado", fechaSistema()).build();
	...
	public static void sendToSession(Session session, JsonObject message) {
		try {
			...
			sessions.remove(session);
			Logger.getLogger(KPISessionHandler.class.getName()).log(Level.SEVERE, null, ex);
		}
	}
}
